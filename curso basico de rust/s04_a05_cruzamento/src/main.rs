/* 	Projeto do Cruzamento Automático - versão s4

	Simula um carro em uma pista simples
*/
...
impl Carro {
	...
	fn new( placa:String, via:char, acel:f64) -> Self {
		...
		assert!( acel >= ACELERACAO_MINIMA  &&  acel <= ACELERACAO_MAXIMA, "   Aceleração inválida: @{} {}", placa, acel );
		...
		Self {
			...
			pos_atual: if via=='H' {-VIAH_PERIMETRO} else {-VIAV_PERIMETRO},
	...
	fn mostra( &self) {
		println!("@{} na posição {}{}, velocidade {}, aceleração {}",
	...
	fn tick( &mut self, tickms:f64) {
		...
		self.pos_atual = self.pos_atual + self.vel_atual * ( tickms/1000.0 )
		...
		if self.pos_atual < pos_anterior {		// Não anda para tras
			self.pos_atual = pos_anterior;
		}

		if self.vel_atual < 0.0 {				// Não anda para tras
			self.vel_atual = 0.0;
		}
...
impl Transito {
	...
	fn new() -> Self {
		Self {
			...
			carros_via_h: [ Carro::new(String::from("AAA0000"), 'H', 0.0),
							Carro::new(String::from("AAA0000"), 'H', 0.0),
							Carro::new(String::from("AAA0000"), 'H', 0.0),
							Carro::new(String::from("AAA0000"), 'H', 0.0)],
			num_carros_criados_v: 0,
			num_carros_sairam_v: 0,
			carros_via_v: [ Carro::new(String::from("AAA0000"),'V', 0.0),
	...
	fn chega_carro( &mut self, via:char, acel:f64) -> bool {
		assert!( via=='H' || via=='V', "   chega_carro recebeu via {}", via);

		let jah_tem = if via=='H' {self.num_carros_criados_h} else {self.num_carros_criados_v};
...
fn simula_carros() {
	...
	loop {
		...
		if tempo_ateh_proxima_chegada <= 0.0 {
			let acel:f64 = 0.0;
			assert!( transito.chega_carro( 'H', ACELERACAO_MAXIMA), "Falha em chegar um carro via H");
			assert!( transito.chega_carro( 'V', ACELERACAO_MAXIMA), "Falha em chegar um carro via V");
