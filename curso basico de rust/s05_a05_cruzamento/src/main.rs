/* 	Projeto do Cruzamento Automático - versão s5

	Simula carros em pista simples
	
	Novidades: Via (enum,match,clone), ocorreu_colisao (Option,match)
*/
...
impl Carro {
	...
	fn new( placa:String, via:Via, acel:f64) -> Self {
		...
		Self {
			...
			pos_atual: match via {					// Posso usar aqui pois foi clonado antes
...
fn simula_carros() {
	...
	loop {
		...
		if tempo_ateh_proxima_chegada <= 0.0 {
			let _acel:f64 = 0.0;
			assert!( transito.chega_carro( Via::ViaH, ACELERACAO_MAXIMA), "Falha em chegar um carro via H");
			assert!( transito.chega_carro( Via::ViaV, ACELERACAO_MAXIMA), "Falha em chegar um carro via V");
