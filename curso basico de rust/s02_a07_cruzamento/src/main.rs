/* 	Projeto do Cruzamento Automático - versão s2

	Simula um carro sozinho em uma pista simples
*/
...
fn dentro_cruzamento(posicao:f64, comprimento:f64, via:char) -> bool {
	return posicao > 0.0  &&  
			posicao <= comprimento + if via=='H' { VIAV_LARGURA } else { VIAH_LARGURA };
}
...
fn main() {
	...
	simula_carros( 'H', ACELERACAO_MAXIMA/10.0, 
		'H', ACELERACAO_MAXIMA );
